...
or returned in any form as the result of any “get” or “search” request, or an action on it is undone via any call to either of the DocumentStore.undo methods.
    ...
    public void stage4TestSetDocLastUseTimeOnGet() throws IOException {
        ...
        Document doc = store.get(this.uri1);
        long first = doc.getLastUseTime();
        doc = store.get(this.uri1);
    ...
    public void stage4TestUpdateDocLastUseTimeOnOverwrite() throws IOException {
        ...
        Document doc2 = store.get(this.uri1);
    ...
    public void stage4TestMaxDocCountViaPut() throws IOException {
        ...
        assertNull(store.get(this.uri1),"uri1 should've been pushed out of memory when uri3 was inserted");
        assertNull(store.get(this.uri2),"uri2 should've been pushed out of memory when uri4 was inserted");
    ...
    public void stage4TestMaxDocCountViaSearch() throws IOException {
        ...
        assertNull(store.get(this.uri2),"uri2 should not still be in memory");
    }
    ...
    public void stage4TestUndoAfterMaxDocCount() throws IOException {
        ...
        assertNull(store.get(this.uri1),"uri1 should've been pushed out of memory");
        //undo the put - should eliminate doc4, and only uri2 and uri3 should be left
        store.undo();
        assertNull(store.get(this.uri4),"uri4 should be gone due to the undo");
        assertNull(store.get(this.uri1),"uri1 should NOT have reappeared once booted out of memory");
        assertNotNull(store.get(this.uri2),"uri2 should still be in memory");
        assertNotNull(store.get(this.uri3),"uri3 should still be in memory");
    }
    ...
    public void stage4TestMaxDocBytesViaSearch() throws IOException {
        ...
        assertNotNull(store.get(this.uri4),"uri4 should still be in memory");
        //uri2 should've been pushed out of memory
        assertNull(store.get(this.uri2),"uri2 should've been pushed out memory");
    }
    ...
    public void stage4TestUndoAfterMaxBytes() throws IOException {
        ...
        assertNotNull(store.get(this.uri2),"uri2 should still be in memory");
        assertNotNull(store.get(this.uri3),"uri3 should still be in memory");
        assertNull(store.get(this.uri1),"uri1 should NOT reappear after being pushed out of memory");
    }
    ...
    //stage 1 tests
    @Test
    public void testPutBinaryDocumentNoPreviousDocAtURI() throws IOException, URISyntaxException {
        DocumentStore store = new DocumentStoreImpl();
        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()), uri1, DocumentStore.DocumentFormat.BINARY);
        assertTrue(returned == 0);
    }

    @Test
    public void testPutTxtDocumentNoPreviousDocAtURI() throws IOException, URISyntaxException {
        DocumentStore store = new DocumentStoreImpl();
        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()), uri1, DocumentStore.DocumentFormat.TXT);
        assertTrue(returned == 0);
    }
    ...
    public void testPutNewVersionOfDocumentBinary() throws IOException {
        ...
        DocumentStore store = new DocumentStoreImpl();
        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()),this.uri1, DocumentStore.DocumentFormat.BINARY);
        assertTrue(returned == 0);
        ...
        returned = store.put(new ByteArrayInputStream(this.txt2.getBytes()),this.uri1, DocumentStore.DocumentFormat.BINARY);

        assertEquals(expected, returned,"should return hashcode of the old document");
        assertArrayEquals(this.txt2.getBytes(),store.get(this.uri1).getDocumentBinaryData(),"failed to return correct data");
    }
...
//        //put the first version
//        DocumentStore store = new DocumentStoreImpl();
//        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()),this.uri1, DocumentStore.DocumentFormat.TXT);
//        assertTrue(returned == 0);
//        assertEquals(this.txt1,store.get(this.uri1).getDocumentTxt(),"failed to return correct text");
//
//        //put the second version, testing both return value of put and see if it gets the correct text
//        returned = store.put(new ByteArrayInputStream(this.txt2.getBytes()),this.uri1, DocumentStore.DocumentFormat.TXT);
//        assertTrue(Utils.calculateHashCode(this.uri1, this.txt1,null) == returned,"should return hashcode of old text");
//        assertEquals(this.txt2,store.get(this.uri1).getDocumentTxt(),"failed to return correct text");
//    }
    ...
    public void testGetTxtDoc() throws IOException {
        ...
        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()),this.uri1, DocumentStore.DocumentFormat.TXT);
        assertTrue(returned == 0);
        assertEquals(this.txt1,store.get(this.uri1).getDocumentTxt(),"did not return a doc with the correct text");
    }
    ...
    public void testGetTxtDocAsBinary() throws IOException {
        ...
        int returned = store.put(new ByteArrayInputStream(this.txt1.getBytes()),this.uri1, DocumentStore.DocumentFormat.TXT);
        assertTrue(returned == 0);
        assertNull(store.get(this.uri1).getDocumentBinaryData(),"a text doc should return null for binary");
    }

    @Test
    public void testGetBinaryDocAsBinary() throws IOException {
        DocumentStore store = new DocumentStoreImpl();
        int returned = store.put(new ByteArrayInputStream(this.txt2.getBytes()),this.uri2, DocumentStore.DocumentFormat.BINARY);
        assertTrue(returned == 0);
        assertArrayEquals(this.txt2.getBytes(),store.get(this.uri2).getDocumentBinaryData(),"failed to return correct binary array");
    }
    ...
    public void testGetBinaryDocAsTxt() throws IOException {
        ...
        int returned = store.put(new ByteArrayInputStream(this.txt2.getBytes()),this.uri2, DocumentStore.DocumentFormat.BINARY);
        assertTrue(returned == 0);
        assertNull(store.get(this.uri2).getDocumentTxt(),"binary doc should return null for text");
    }
    ...
    public void testDeleteDoc() throws IOException {
        ...
        assertNull(store.get(this.uri1),"calling get on URI from which doc was deleted should've returned null");
    }
    ...
    public void testDeleteDocReturnValue() throws IOException {
        ...
        assertEquals(true,store.delete(this.uri1),"failed to return true when deleting a document");
        //should return false if I try to delete the same doc again
        assertEquals(false,store.delete(this.uri1),"failed to return false when trying to delete that which was already deleted");
        //should return false if I try to delete something that was never there to begin with
        assertEquals(false,store.delete(this.uri2),"failed to return false when trying to delete that which was never there to begin with");
    }
}
